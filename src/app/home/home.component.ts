...
@Component({
  ...
  imports: [CommonModule, RouterLink],
  templateUrl: './home.component.html',
  styleUrls: ['./home.component.css'] // Corrected to use the correct `styleUrls` syntax
})
export class HomeComponent {
  trendingProjects: Project[] = [];
  loading = false;
  ...
  async viewTrending() {
    try {
      ...
      // Check if projects is an array before using slice
      if (Array.isArray(projects)) {
        this.trendingProjects = projects.slice(0, 6);
        console.log("Loaded trending projects");
      } else {
        console.error('Invalid response from fetchTopTrendingProjects:', projects);
      }
    } catch (error) {
      console.error('Error loading trending projects:', error);
    } finally {
